  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 text-center">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-2">{t("abandon_modal.title")}</h3>
        <p className="text-sm text-gray-500 mb-6 leading-relaxed">
          {t("abandon_modal.desc")}
        </p>
        <div className="flex flex-col gap-2">
          ...
          >
            <ArrowRight className="w-4 h-4 mr-2" /> {t("abandon_modal.btn_continue")}
          </Button>
          <Button variant="ghost" onClick={onExit} className="text-gray-500 text-sm">
            {t("abandon_modal.btn_exit")}
          </Button>
        </div>
      </div>
    </div>
